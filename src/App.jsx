...
function DispatchPage({ data, setData, toast }) {
  ...
  // 只看待採購/審核中的訂單，按下單時間排序（先下先配）
  const activeOrders = Array.from(new Map(data.orders.map(o=>[o.id,o])).values())
    .filter(o => ["pending","pending_review","bought"].includes(o.status) && !o.archived)
  ...
  activeOrders.forEach(o => {
    (o.items||[]).forEach((it, itemIdx) => {
      const key = it.name;
      if (!itemMap[key]) {
        itemMap[key] = { name:key, image:it.image||"", totalNeeded:0, orders:[] };
      }
      itemMap[key].totalNeeded += (it.qty||1);
      const isDispatched = (o.item_dispatch||[])[itemIdx] === true;
      itemMap[key].orders.push({
        orderId: o.id, orderNo: o.no, itemIdx,
  ...
  const saveStock = name => {
    ...
    toast(`+${addVal} 個，「${name.slice(0,10)}」庫存共 ${newTotal} 個`);
  };
  ...
  // 單一品項配貨
  const dispatchItem = async (orderId, orderNo, itemIdx, itemName) => {
    const key = `${orderId}_${itemIdx}`;
    setDispatching(p => ({...p, [key]: true}));

    const order = data.orders.find(o => o.id === orderId);
    const dispatch = [...(order?.item_dispatch || Array((order?.items||[]).length).fill(false))];
    while (dispatch.length < (order?.items||[]).length) dispatch.push(false);
    dispatch[itemIdx] = true;

    // 如果這筆訂單所有品項都已配貨，狀態改為 bought
    const allDispatched = dispatch.every(v => v === true);
    const updateData = { item_dispatch: dispatch };
    if (allDispatched) updateData.status = "bought";
    ...
    if (error) {
      ...
      setData(d => ({
        ...
        orders: d.orders.map(o => o.id === orderId
          ? {...o, item_dispatch: dispatch, ...(allDispatched ? {status:"bought"} : {})}
          : o
        )
      }));
      toast(allDispatched
        ? `✅ #${orderNo} 全部配貨完成，狀態更新為已採購`
        : `✅ #${orderNo}「${itemName.slice(0,10)}」已配貨`
      );
    }
    setDispatching(p => ({...p, [key]: false}));
  };

  const exportDispatch = () => {
    const header = ["商品","客人","下單日期","需求數量","已配貨","訂單號"];
    const rows = [];
    allItems.forEach(item => {
      item.orders.forEach((o, idx) => {
        rows.push([
          idx===0 ? item.name : "",
          o.customerName, o.date, o.qty,
          o.isDispatched ? "是" : "否",
          "#"+o.orderNo
        ]);
      });
  ...
  if (!allItems.length) return (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
      ...
      <Card style={{ textAlign:"center", padding:"48px 0" }}>
        <div style={{ fontSize:36, marginBottom:8 }}>📭</div>
        <div style={{ color:C.muted }}>目前沒有待配貨的訂單</div>
      </Card>
    </div>
  );
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div>
          ...
          <div style={{ fontSize:12, color:C.muted, marginTop:2 }}>
            {allItems.length} 種商品 · {activeOrders.length} 筆訂單 · 按下單順序
          </div>
        </div>
      ...
      <div style={{ background:C.amberBg, borderRadius:12, padding:"10px 14px", fontSize:12, color:C.amber, border:`1px solid ${C.amber}30` }}>
        💡 每個品項可單獨配貨；所有品項配完後，訂單狀態自動改為「已採購」
      </div>
        ...
        return (
          <Card key={item.name} style={{ overflow:"hidden" }}>
            ...
            <div style={{ display:"flex", gap:8, marginBottom:14, alignItems:"flex-end" }}>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:11, color:C.muted, marginBottom:4 }}>本次進貨數量（累加）</div>
                <input type="number" value={stockInput[item.name]||""} placeholder="輸入進貨數量"
                  onChange={e=>setStockInput(p=>({...p,[item.name]:e.target.value}))}
                  style={{ width:"100%", background:C.surface, border:`1.5px solid ${C.border}`, borderRadius:10, padding:"8px 12px", fontSize:14, outline:"none" }}
                  onFocus={e=>e.target.style.borderColor=C.accent}
                  onBlur={e=>e.target.style.borderColor=C.border}
                />
              </div>
              <button onClick={()=>saveStock(item.name)}
                style={{ background:C.accent, color:"#fff", border:"none", borderRadius:10, padding:"9px 16px", fontSize:13, fontWeight:600, cursor:"pointer" }}>
                + 加入庫存
              </button>
              {stock > 0 && (
                <button onClick={()=>resetStock(item.name)}
                  style={{ background:C.redBg, color:C.red, border:`1px solid ${C.red}30`, borderRadius:10, padding:"9px 12px", fontSize:12, cursor:"pointer" }}>
                  清零
                </button>
              )}
            </div>
            ...
            {/* 按下單順序條列客人品項 */}
            <div style={{ borderTop:`1px solid ${C.border}`, paddingTop:12 }}>
              <div style={{ fontSize:11, color:C.muted, fontWeight:600, letterSpacing:.3, marginBottom:8 }}>按下單順序配貨</div>
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                {item.orders.map((o, idx) => {
                  const key = `${o.orderId}_${o.itemIdx}`;
                  const isLoading = dispatching[key];
                  return (
                    <div key={key} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 12px", borderRadius:10,
                      background: o.isDispatched ? C.greenBg : C.bgDeep,
                      border:`1px solid ${o.isDispatched ? C.green+"30" : C.border}`
                    }}>
                      <div style={{ width:22, height:22, borderRadius:"50%", background:o.isDispatched?C.green:C.accent, color:"#fff", display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:700, flexShrink:0 }}>
                        {idx+1}
                      </div>
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ fontSize:12, fontWeight:600 }}>{o.customerName}</div>
                        <div style={{ fontSize:10, color:C.muted }}>#{o.orderNo} · {o.date} · ×{o.qty} 個</div>
                      </div>
                      {o.isDispatched
                        ? <span style={{ fontSize:11, padding:"4px 12px", borderRadius:99, background:C.greenBg, color:C.green, fontWeight:600 }}>✓ 已配貨</span>
                        : <button onClick={()=>dispatchItem(o.orderId, o.orderNo, o.itemIdx, item.name)} disabled={isLoading}
                            style={{ fontSize:12, padding:"6px 14px", borderRadius:99, background:isLoading?C.faint:C.accent, color:"#fff", border:"none", cursor:isLoading?"not-allowed":"pointer", fontWeight:600 }}>
                            {isLoading?"配貨中...":"✓ 配貨"}
                          </button>
                      }
                    </div>
